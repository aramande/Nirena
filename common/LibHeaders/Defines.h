...
<A HREF="modules.html">Click here for accesing the Menu</A>
*/
...
to the screen without having to use this advanced methods directly. This method are only useful for advanced 
...
/**
 * \defgroup Estructuras Additional structures
 * \ingroup Types
 */
/*@{*/
...
/*!
IND_Image objects can have different formats depending on the number of colours. To have \b truecolor, the range of colors than 
human eye can perceive,  3 bytes per pixel are necessary. These are   \b R (Red), \b G (Green) 
and \b B (Blue) bytes.
...
For example using gray range (they only use oner byte per pixel).
...
::IND_ALPHA. This type consumes more memory that ::IND_OPAQUE but both will be equally faster
when rendering.
...
You can try diferents values to the source and to the destination and you can get a big mount of 
diverse efects as much aditive as subtracttive  (relative to RGBA values) and a great visual impact.
*/
...
we resize a surface, the quality of the image will be better o worse depending on the filter used.
...
if are the resources are correctly loaded/freed. Please, check this file if any problem happen after the execution of
