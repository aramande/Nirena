...
private:
	...
	IND_3dMesh		*mMe;		// Puntero a una malla
	...
	float mZ;					// Coordenada z
	float mAngleX;				// Ángulo eje x
	float mAngleY;				// Ángulo eje y
	float mAngleZ;				// Ángulo eje z
	float mScaleX;				// Escalado x
	float mScaleY;				// Escalado y	
	float mScaleZ;				// Escalado z
	bool mCull;					// Back Face Culling Activado/Desactivado
	IND_Filter mFilter;			// Tipo filtro (0 = Nearest Point || 1 = Linear)
	IND_Matrix	mMat;			// Matriz de mundo
	...
	byte mFadeA;				// Fade to color A (cantidad de fade)
	IND_BlendingType mSo;		// Fuente de blending
	IND_BlendingType mDs;		// Destino de blending

	// Animations attributes
	bool mChangeSequenceFlag;	// Se activa cuando se cambia la secuencia
	int mSequence;				// Num de secuencia de animación que se quiere dibujar
